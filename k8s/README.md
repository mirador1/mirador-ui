# `k8s/` — Kubernetes manifests for the Angular frontend
...
## How this relates to `mirador-service`'s `k8s/`

The Spring Boot backend repository has its own `k8s/` directory with its
own Deployment, Service, HPA, ConfigMap, Ingress, etc. Both projects share
the **same K8s namespace (`app`)** and the **same Ingress** — the
backend's `k8s/ingress.yaml` routes `/api/*` → backend and `/*` → frontend
on the shared hostname (`mirador1.duckdns.org`). So this directory only
owns the frontend resources; the Ingress lives in `mirador-service/k8s/`.
...
docker-build → push image to registry.gitlab.com/mirador1/mirador-ui:$SHA
  ↓
deploy:gke → kubectl apply k8s/frontend/{deployment,service}.yaml
...
- **Ingress** → belongs in `mirador-service/k8s/ingress.yaml` (single
