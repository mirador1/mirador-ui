...
| `sonarcloud` | `allow_failure: true` | **REMOVED** (2026-04-25, [3e80d81](https://gitlab.com/mirador1/mirador-ui/-/commit/3e80d81)) | 🟡 UNVERIFIED | Shield removed in wave 11 commit; pipeline #461 had sonarcloud=skipped (path filter); pipelines #463/#464 currently running on main (post-multi-platform image fix [46acc46](https://gitlab.com/mirador1/mirador-ui/-/commit/46acc46)) — first real validation of post-fix sonarcloud |
...
If pipelines #463 or #464 land sonarcloud=success on main, the closure
is confirmed. If they fail again, the next step is :
- Re-add a shield with a fresh dated TODO 2026-05-15 (20d)
  documenting the new failure mode
- Open a follow-up MR to either downgrade the JS sensor concurrency
  further or pin a known-good sonar-scanner-cli base version
