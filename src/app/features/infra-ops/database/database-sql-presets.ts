/**
 * database-sql-presets.ts — 35 preset SQL queries for the Database page.
 *
 * Extracted 2026-04-24 under Phase B-7-7b file-length hygiene : the parent
 * `database.component.ts` dropped the 278-line `sqlPresetCategories`
 * literal so the component file stays focused on signals + SQL execution
 * behaviour (going from 522 → ~100 LOC).
 *
 * Pattern : "1 concern per file" from CLAUDE.md → each data-heavy array
 * sits in its own TypeScript module alongside the consuming component.
 * Matches `database-health-checks.ts` (sibling), `customers-data.ts`,
 * `quality-helpers.ts` already landed under this rule.
 *
 * The 5 categories (customer / diagnostics / schema / investigation /
 * performance) mirror the Database tabs : clicking a preset runs its
 * `query` against pgweb and displays the result in the SQL Explorer.
 */
import type { DbTab } from './database-types';

/** One preset row : icon + short name + tooltip tip + raw SQL. */
...
/** One tab's worth of preset queries. `id` matches the {@link DbTab} route. */
export interface SqlPresetCategory {
  id: DbTab;
  label: string;
  presets: SqlPreset[];
}
...
/**
 * 35 read-only PostgreSQL queries grouped by investigation intent :
 *
 * - `customer` (4) — CRUD verification, idempotency sanity checks.
 * - `diagnostics` (13) — full pg_stat_* coverage for day-to-day health.
 * - `schema` (4) — Flyway history + schema introspection.
 * - `investigation` (7) — production incident triage (locks, long
 *   transactions, rollback-rate spikes, temp-file spills).
 * - `performance` (7) — tuning + EXPLAIN + autovacuum status.
 *
 * Keep queries ordered within each category from "cheap / broad-signal"
 * to "expensive / targeted" so users scan top-to-bottom during triage.
 */
export const SQL_PRESET_CATEGORIES: SqlPresetCategory[] = [
  {
    ...
    presets: [
      ...
      },
      {
        ...
        query: 'SELECT id, name, email, created_at FROM customer ORDER BY created_at DESC LIMIT 10',
      },
      {
        icon: '👥',
        name: 'Duplicates',
