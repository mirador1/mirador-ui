    ...
    },
    {
      ...
      presets: [
        ...
        },
        {
          icon: '📏',
          name: 'Table sizes',
          tip: 'Size of each table including data and indexes. Identifies the largest tables that may need partitioning or archiving. Compare data_size vs index_size — heavy index overhead may indicate too many indexes.',
          query:
        ...
        },
        {
          icon: '📊',
          name: 'Index usage',
          tip: 'How often each index is used (scans) and how many tuples it reads. Indexes with high scans are essential. Compare with "Unused indexes" to find candidates for removal.',
          query:
        ...
        },
        {
          icon: '🗑️',
          name: 'Unused indexes',
          tip: 'Indexes that have never been used since the last stats reset (excluding primary keys). These waste disk space and slow down writes (INSERT/UPDATE). Safe to drop if confirmed unused.',
          query:
        ...
        },
        {
          ...
          tip: 'Percentage of data reads served from PostgreSQL shared_buffers cache vs disk. Should be >99% in production. Low ratio = insufficient shared_buffers or working set too large for memory.',
          query:
        ...
        },
        {
          ...
          tip: 'Tables with sequential scans (full table reads). High seq_scan with low idx_scan = missing index. avg_rows_per_seq_scan shows the cost of each scan — large tables with frequent seq scans need indexes.',
          query:
        ...
        },
        {
          icon: '🔒',
          name: 'Lock waits',
          tip: 'Queries currently blocked by another transaction holding a lock. Shows both the blocked and blocking queries. Use during chaos testing (concurrent writes) to observe lock contention.',
          query:
        ...
        },
        {
          icon: '💾',
          name: 'Database size',
          tip: 'Total size of the current database on disk. Includes all tables, indexes, and TOAST data. Monitor growth over time — unexpected growth may indicate missing cleanup or bloat.',
          query:
        ...
        },
        {
          ...
          tip: 'Breakdown of connection states (active, idle, idle in transaction). Too many "idle in transaction" connections indicate unclosed transactions — they hold locks and prevent autovacuum.',
          query:
        ...
        },
        {
          ...
          tip: 'Tables where sequential scans outnumber index scans — likely missing indexes. The seq_to_idx_ratio shows how many times more seq scans than idx scans occur. Ratio > 1 with >100 rows = add an index.',
          query:
        ...
        },
        {
          icon: '🧹',
          name: 'Bloat estimate',
          tip: 'Dead tuples (deleted/updated rows not yet vacuumed) per table. High dead_pct means the table is bloated — autovacuum may not be keeping up. Check last_vacuum and last_autovacuum timestamps.',
          query:
        ...
        },
        {
          ...
          tip: 'Active replication connections (streaming replicas). Empty result = no replicas configured. In production, shows lag (sent_lsn vs replay_lsn) to monitor replication health.',
          query: 'SELECT * FROM pg_stat_replication',
        },
        {
          icon: '📁',
          name: 'Tablespace usage',
          tip: 'Disk space used by each tablespace. Default tablespace (pg_default) holds all user data. Monitor to prevent disk exhaustion — PostgreSQL stops accepting writes when disk is full.',
          query:
      ...
      presets: [
        {
          icon: '📜',
          name: 'Flyway history',
          tip: 'All Flyway schema migrations applied to this database, with version, description, execution time, and success status. Verify that all migrations ran successfully after deployment.',
          query:
        ...
        },
        {
          icon: '🏗️',
          name: 'Columns',
          tip: 'Full schema of all tables: column names, data types, nullability, and default values. Use to verify the customer table structure matches the JPA entity (id BIGSERIAL, name VARCHAR, email VARCHAR, created_at TIMESTAMP).',
          query:
        ...
        },
        {
          icon: '🔐',
          name: 'Constraints',
          tip: 'All constraints: primary keys (p), unique (u), foreign keys (f), check (c). Verify that the customer table has a primary key on id and any unique constraints on email.',
          query:
      ...
      presets: [
        {
          ...
          tip: 'Queries running for more than 5 seconds. These block connections and may cause HikariCP pool exhaustion. Kill with pg_terminate_backend(pid) if stuck.',
          query:
        ...
        },
        {
          ...
          tip: 'Processes waiting for a lock held by another process. In a deadlock, both sides appear here. Identify the blocking PID and terminate it if necessary.',
          query:
        ...
        },
        {
          ...
          tip: 'Connections stuck in "idle in transaction" state. These hold locks and prevent autovacuum. Transactions open >1 minute are likely bugs (missing commit/rollback). Check application connection handling.',
          query:
        ...
        },
        {
          ...
          tip: 'Commits and rollbacks per table since last stats reset. High rollback rate indicates application errors or contention. Compare with HTTP error rate in Prometheus.',
          query:
        ...
        },
        {
          ...
          tip: 'Read vs write patterns per table. High n_tup_ins = write-heavy. High seq_scan with low idx_scan = missing index. n_tup_hot_upd = HOT updates (good for performance).',
          query:
        ...
        },
        {
          ...
          tip: 'The oldest open transaction in the database. Long-running transactions prevent VACUUM from reclaiming dead rows (bloat). Should normally be under 1 minute.',
          query:
        ...
        },
        {
          ...
          tip: 'Queries that spill to disk (temp files) because work_mem is too small. Large temp_bytes = slow queries. Increase work_mem or optimize the query to avoid sorts and hash joins on large datasets.',
          query:
      ...
      presets: [
        {
          ...
          tip: 'Key performance-related PostgreSQL settings: shared_buffers (cache size), work_mem (sort memory), max_connections, effective_cache_size. Compare with recommended values for your workload.',
          query:
        ...
        },
        {
          ...
          tip: 'Index size vs table size ratio. If indexes are larger than the table, consider removing unused indexes. Also shows index scan count — zero scans means the index is never used.',
          query:
        ...
        },
        {
          ...
          tip: 'Tables where sequential scans dominate. A table with 1000+ rows and more seq_scans than idx_scans almost certainly needs an index. The cost column shows estimated rows read per scan.',
          query:
        ...
        },
        {
          ...
          tip: 'Heap-Only Tuple (HOT) updates avoid index updates — much faster. Low HOT ratio on frequently updated tables means the updated columns are indexed. Consider removing indexes on frequently updated columns.',
          query:
        ...
        },
        {
          ...
          tip: 'When autovacuum last ran on each table and how many dead tuples are waiting. Tables with many dead tuples and old last_autovacuum need attention — autovacuum may be throttled or blocked.',
          query:
        ...
        },
        {
          icon: '💿',
          name: 'WAL statistics',
          tip: 'Write-Ahead Log stats: how many WAL records are generated. High WAL volume = write-heavy workload. Affects replication lag and backup speed. Monitor after enabling full_page_writes.',
          query: 'SELECT * FROM pg_stat_wal',
        },
        {
          icon: '🏗️',
          name: 'EXPLAIN customer query',
          tip: 'Query execution plan for the main customer list query. Shows whether PostgreSQL uses an index scan or sequential scan, estimated costs, and actual row counts. Run after adding/removing indexes.',
          query:
