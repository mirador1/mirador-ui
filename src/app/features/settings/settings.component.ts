...
export class SettingsComponent {
  ...
  loadInfo(): void {
    ...
    this.http.get(`${base}/actuator/info`).subscribe({
      next: v => { this.actuatorInfo.set(v); this.loading.set(false); },
      error: () => { this.error.set('Could not reach Actuator endpoints'); this.loading.set(false); }
    });
      ...
      next: v => {
        ...
        for (const src of v.propertySources ?? []) {
          for (const [key, val] of Object.entries(src.properties ?? {})) {
            if (key.includes('rate') || key.includes('timeout') || key.includes('circuit') ||
                key.includes('resilience') || key.includes('kafka') || key.includes('bucket') ||
                key.includes('server.port') || key.includes('spring.application')) {
  ...
  callEndpoint(path: string): void {
    ...
    if (isText) {
      this.http.get(`${this.env.baseUrl()}${path}`, { responseType: 'text' }).subscribe({
        next: v => { this.endpointResult.set(v); this.endpointLoading.set(false); },
        error: e => { this.endpointResult.set(`Error ${e.status}: ${e.message}`); this.endpointLoading.set(false); }
      });
      return;
    }
  ...
  loadLoggers(): void {
    ...
    this.http.get<{ loggers: Record<string, { effectiveLevel: string }> }>(`${this.env.baseUrl()}/actuator/loggers`).subscribe({
      next: v => {
  ...
  setLoggerLevel(name: string, level: string): void {
    this.http.post(`${this.env.baseUrl()}/actuator/loggers/${name}`, { configuredLevel: level }).subscribe({
      next: () => {
